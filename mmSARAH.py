...
class mmSARAH:
    ...
    def fit(self, X_train, y_train, X_test =None, y_test = None, batch_size = 30, L = 0.1, loss = 'sigmoind', eval = True,inertial = False, m= 1, pr = False):
        ...
        for i in range(n//batch_size):
            ...
            # S[i] = self.grad(self.w, X, y, L, loss)
        ...
        for iter in range(self.maxIter):
            ...
            # eta = self.eta*self.learning_schedule(ttt)
                ...
                if random.sample(range(m),1)[0]!=0:
                    ...
                    beta = min((tp-1)/t,1-1e-5)
                    w_barp = w_bar
                    w_bar = self.w + beta*(self.w - wp)
                    # w_barp = wp + beta*(wp - wpp)

                    grad_b = self.grad(w_bar, X, y, L, loss)
                    grad_bp = self.grad(w_barp, X, y, L, loss)
                    v = v + (grad_b-grad_bp)/batch_size
                    grad = v 
                ...
                wpp = wp
                wp = self.w
                self.w = self.update_w(w_bar + grad/(self.eta*L+L),reg/(self.eta*L+L),loss)
        ...
        for iter in range(self.maxIter):
            ...
            # eta = self.eta*self.learning_schedule(ttt)
                ...
                if eval and full_bach >=1:
                    ...
                    self.Time.append(time.time() - start_time)
                    self.ngrad.append(self[-1] + full_bach)
                    full_bach = 0
                    f_val = self.obj_func(self.w, X_train, y_train, loss)
