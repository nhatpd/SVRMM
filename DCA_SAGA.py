...
class DCA_SAGA:
    ...
    def fit(self, X_train, y_train, X_test = None, y_test = None, batch_size = 30, L = 0.1, loss = 'sigmoind', eval = True, pr = False):
        ...
        for iter in range(self.maxIter):
            ...
            indexes = shuffle(range(n//batch_size))
            ...
            # eta = self.eta*self.learning_schedule(ttt)
                ...
                grad_b = self.grad(self.w, X, y, L, loss)
                grad = v + (grad_b-S[i])/batch_size + self.eta*L*self.w
                # grad = v + (grad_b-S[i])/n + self.eta*L*self.w
                v = v + (grad_b - S[i])/n
                S[i] = grad_b 
